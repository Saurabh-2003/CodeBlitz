...
export const Profile = () => {
  ...
  const [skills, setSkills] = useState(['ReactJS', 'TailwindCSS', 'MongoDB', 'Express'])
  const [languages, setLanguages] = useState([{name:'C++', sol:480}, {name:'MySQL', sol:50}, {name:'Pandas', sol:6}, {name:'Bash', sol:2}])
  return (
    <aside className='flex flex-col p-4 items-center h-fit bg-white w-1/3'>
        <div className='flex h-24 w-full gap-4'>
            <img className=' size-20 bg-red-600 rounded-xl overflow-hidden object-contain' src='/images/placeholder.jpg' />
        ...
        <div className='flex self-start mt-6 gap-2 text-gray-700'>
            ...
            <ul className='flex flex-wrap gap-2'>
                {
                    skills.map((skill) => (
                        <li className=' text-[11px] py-1 px-2 text-stone-700 bg-stone-100 rounded-full'>{skill}</li>
                    ))
                }
            </ul>
        </div>
