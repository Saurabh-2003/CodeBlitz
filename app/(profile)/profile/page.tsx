...
const Profilepage = () => {
  ...
  const navItems = [
    ...
    { name: 'List', path: '/list', icon: <HiOutlineClipboardDocumentList size={25} /> },
    { name: 'Solutions', path: '/Solutions', icon: <GoCheckbox size={25} /> },
    { name: 'Discuss', path: '/Discuss', icon: <PiChatsCircleLight size={25} /> }
  ];

  const dummyData = [
    { title: "Minimum Number of Moves to Seat Everyone", daysAgo: "2 hours ago" },
    ...
    { title: "Minimum Number of Chairs in a Waiting Room", daysAgo: "8 days ago" },
    { title: "Find Common Characters", daysAgo: "8 days ago" },
    { title: "Longest Palindrome", daysAgo: "9 days ago" },
    { title: "Append Characters to String to Make Subsequence", daysAgo: "10 days ago" },
        ...
        <div className='h-fit flex gap-4 flex-col p-4 shadow-md rounded-xl bg-white w-full'>
          <nav>
            <ul className='flex gap-6'>
              {navItems.map(item => (
                <li key={item.path} className={location === item.path ? 'active' : ''}>
          ...
          <ul className='flex text-sm flex-col '>
            {dummyData.map(d => (
              <li className='flex p-4 odd:bg-zinc-100 rounded-xl justify-between items-center' key={d.title}>
