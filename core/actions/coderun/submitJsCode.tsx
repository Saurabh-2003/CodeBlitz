...
export const submitJsCode = async (solution: SolutionProps) => {
  try {
    ...
    const problem = await db.problem.findUnique({
      where: { id: problemId },
      select: { jsDriver: true, inputs: true, outputs: true },
    });
    ...
    const basePath = "/tmp";
    const codeFileName = path.join(basePath, `code_${uuidv4()}.js`);
    ...
    if (lang !== "javascript") {
      throw new Error(
        "Unsupported language. Only JavaScript is supported for now.",
      );
    }

    // Run the JavaScript code using Docker
    const runCommand = `docker run --rm -v ${basePath}:${basePath} -w ${basePath} js_service sh -c '
      node ${path.basename(codeFileName)} < ${path.basename(inputFileName)} > ${path.basename(outputFileName)} && \
