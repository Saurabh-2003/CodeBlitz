...
const submitPythonCode = async (solution: SolutionProps) => {
  try {
    ...
    const problem = await db.problem.findUnique({
      where: { id: problemId },
      select: { pythonDriver: true, inputs: true, outputs: true },
    });
    ...
    const basePath = "/tmp";
    const codeFileName = path.join(basePath, `code_${uuidv4()}.py`);
    ...
    if (lang !== "python") {
      throw new Error(
        "Unsupported language. Only Python is supported for now.",
      );
    }

    // Run the Python code using Docker and pass the input file
    const runCommand = `docker run --rm -v ${basePath}:${basePath} -w ${basePath} python_service sh -c '
      python3 ${path.basename(codeFileName)} < ${path.basename(inputFileName)} > ${path.basename(outputFileName)} && \
      cat ${path.basename(outputFileName)}
    '`;

    console.log("Run Command:", runCommand); // For debugging

    // Execute the command to run the Python program
...
export default submitPythonCode;
