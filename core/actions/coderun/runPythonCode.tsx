...
export const handlePythonCode = async (userCode: string) => {
  createDir();

  const fileName = `script_${Date.now()}.py`; // Unique filename to avoid overwriting
  const filePath = path.resolve(TEMP_DIR, fileName);
  fs.writeFileSync(filePath, userCode, "utf-8");

  const runCommand = `sudo docker run --rm -i -v ${TEMP_DIR}:/code codeblitz-python_service python ${fileName}`;

  try {
    const outputValue = await runPythonCode(runCommand);
...
const runPythonCode = (runCommand: string): Promise<string> => {
  return new Promise((resolve, reject) => {
    const pythonProcess = spawn(runCommand, {
      shell: true,
